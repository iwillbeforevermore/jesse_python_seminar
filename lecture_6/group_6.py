...
class nestedSphere_cap(capacitor):
    def __init__(self,radiusA,radiusB,ref):
        self.radiusA = float(radiusA)
        self.radiusB = float(radiusB)
        self.sep = abs(radiusA-radiusB)
        self.capacitance = E0*pi*4/((1.0/radiusA)-(1.0/radiusB))
        self.ref = ref
        self.q = 0.0
        ref[self] = self.q
